...
-j               Use Javascript output, creating DefineAllDevices(), DefineDevice(d), SendCommands(d,c1,...)
...
perl $prg -C devices/channels.txt -c DCT2524 devices/DCT2524 devices/VSX9300 > cablebox-and-amp.html
...
sub process_config {
  ...
  $codefor_code .=  "var d = CodeFor['$remote_name'] = new Array()\n";
  for my $aref (@codes) {
    my ($k, $v) = @$aref;
    $codefor_code .= "d['$k'] = '$v';\n";
    ...
    if ($opt_r) {
      ...
      print "<a href='#' onClick='DevCmd(\"$remote_name\", \"$k\")';' title='$cmd'>".html_escape($k)."</a> &nbsp;\n";
    }
  }
  ...
  if ($opt_c && ($show_channel_guide_for_dev{$remote_name})) {
	...
	my $devcmd = "DevCmd(\"$remote_name\", \"" . join(" ", split(//, $num)) ."\");";
	if ($opt_r) {
	  print "<a href='$baseurl". uri_escape($cmd) ."'>".html_escape($name)."</a> &nbsp; \n";
	} else {
	  print "<a href='#' onClick='$devcmd' title='$cmd'>".html_escape($name)."</a> &nbsp;\n";
	}
      }
    }
  }
...
var CodeFor = new Array();
function DevCmd(dev, s) {
        cmd("devcmd " + dev + CodeFromSymbolicCmd(dev, s));
}
</script>
END_HEADER
}
...
sub output_footer {
  ...
  print "function DefineAllDevices() {\n";
  for my $sd (@$aref) {
    print "cmd(\"$sd\");\n";
  }
  print "}\n";
  print $codefor_code;
  print <<EOFUNCTION;
function CodeFromSymbolicCmd(dev, cmdstr) {
  var answer = "";
  var cmds = cmdstr.split(" ");
  for (var i=0; i < cmds.length; ++i) {
    var code = CodeFor[dev][cmds[i]];
...
perl irconf-to-html.pl -c DCT2524 -C devices\channels.txt -nSonyTV,PioneerAmp, devices\RM-Y168 devices\VSX9300 devices\DCT2524 > out.html
